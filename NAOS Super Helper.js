...
// @version      2.1.0
// @updateURL    https://raw.githubusercontent.com/ShapArt/naos-super-helper/main/NAOS%20Super%20Helper.js
// @downloadURL  https://raw.githubusercontent.com/ShapArt/naos-super-helper/main/NAOS%20Super%20Helper.js
...
(function () {
  ...
  /**
   * Changelog:
   * 2.1.0 (2025-12-26): автообновление из GitHub, чистый UI без градиентов, фикс закрытия
   * гайда, автопереключение вкладки по странице, корректный тип статьи, дисклеймер-цвет
   * по бренду, улучшенный тумблер свернуть/развернуть.
  ...
  Core.Updater = (function () {
    const UPDATE_URL =
      "https://raw.githubusercontent.com/ShapArt/naos-super-helper/main/NAOS%20Super%20Helper.js";
    const CHECK_KEY = "UPDATE_LAST_CHECK_V1";
    const CHECK_INTERVAL = 1000 * 60 * 60 * 12; // раз в 12 часов

    function parseVersion(text) {
      if (!text) return "";
      const meta = text.match(/@version\s+([\d.]+)/i);
      if (meta) return (meta[1] || "").trim();
      const varMatch = text.match(/version[:=]\s*["']([\d.]+)["']/i);
    ...
    async function check(force = false) {
      const now = Date.now();
      const last = Core.State?.getJson(CHECK_KEY, 0) || 0;
      if (!force && last && now - last < CHECK_INTERVAL) return null;
      Core.State?.setJson(CHECK_KEY, now);
      try {
        ...
        if (remote && isNewer(remote, SH.version)) {
          Core.Log.info("UPDATE", `Доступна новая версия v${remote}`);
          Core.Toast.show(
            `Доступна новая версия NAOS Super Helper v${remote}`,
            "info",
          );
          const btn = document.querySelector("[data-nh-update]");
          if (btn) {
            btn.style.display = "inline-flex";
            btn.dataset.targetVersion = remote;
            btn.textContent = `Обновить v${remote}`;
          }
          return remote;
        }
      } catch (e) {
...
6) Панель сама понимает, где ты (Журнал / Баннер / Акция) и открывает нужную вкладку.
...
Alt: Баннер -20% Bioderma`;
  Core.UI = (function () {
    ...
    function injectStyles() {
      ...
      .nh-title { font-size: 13px; font-weight: 700; color: var(--nh-text); letter-spacing: 0.01em; }
      .nh-version { font-size: 11px; color: var(--nh-muted); margin-left: 4px; }
      .nh-author { font-size: 11px; color: #0ea5e9; margin-left: 6px; opacity: 0.9; }
      ...
      .nh-action-btn.ghost {
        background: var(--nh-accent-ghost);
        color: var(--nh-text);
      ...
      .nh-action-btn.secondary:hover,
      .nh-btn-secondary:hover {
        background: #f1f5f9;
        box-shadow: 0 8px 18px rgba(15,23,42,0.08);
        color: var(--nh-text);
      }
      .nh-action-btn.ghost:hover {
        background: #e5e9f2;
      }
      .nh-action-btn:active,
      .nh-btn:active {
      ...
      .nh-tab:hover { background: #e2e8f0; transform: translateY(-1px); }
      .nh-tab-active {
        background: var(--nh-accent);
        color: #f8fafc;
      ...
      .nh-tab-panels { position: relative; }
      ...
      .nh-log-item { padding: 4px 0; border-bottom: 1px dashed #e2e8f0; }
      .nh-log-item:last-child { border-bottom: none; }
      .nh-log-item:hover { background: #e7eef6; }
      .nh-log-time { color: #64748b; font-variant-numeric: tabular-nums; margin-right: 4px; }
      .nh-log-area { color: #0ea5e9; margin-right: 4px; }
      .nh-log-level-INFO { color: #334155; font-weight: 600; margin-right: 4px; }
      .nh-log-level-WARN { color: #d97706; font-weight: 700; margin-right: 4px; }
      .nh-log-level-ERROR { color: #b91c1c; font-weight: 700; margin-right: 4px; }
      .nh-log-message { color: #0f172a; }
      .nh-guide-overlay {
        position: absolute;
        inset: 0;
      ...
      .nh-guide-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; gap: 8px; }
      .nh-guide-title { font-size: 14px; font-weight: 800; color: #0f172a; }
      .nh-guide-close {
        cursor: pointer;
        border: 1px solid var(--nh-accent);
        background: var(--nh-accent);
      ...
      .nh-guide-close:hover { background: #111827; transform: translateY(-1px); box-shadow: 0 8px 18px rgba(15,23,42,0.16); }
      .nh-guide-body {
        white-space: pre-wrap;
        font-family: "Inter","SFMono-Regular",Consolas,Menlo,monospace;
        font-size: 12px;
        line-height: 1.6;
        background: #f8fafc;
        border: 1px solid #e2e8f0;
    ...
    function bindActions() {
      ...
      const guideOverlayEl = root.querySelector(".nh-guide-overlay");
      if (guideOverlayEl)
        guideOverlayEl.addEventListener("click", (e) => {
          if (e.target === guideOverlayEl) hideGuide();
        });
      ...
      const updateBtn = root.querySelector("[data-nh-update]");
      if (updateBtn)
        updateBtn.addEventListener("click", () => {
          Core.Updater && Core.Updater.openDownload && Core.Updater.openDownload();
        });
      ...
      root.innerHTML = `
        <div class="nh-header">
          ...
          <div class="nh-header-actions">
            <button class="nh-action-btn ghost" type="button" data-nh-update>Обновить</button>
            <button class="nh-action-btn" type="button" data-nh-guide>Гайд</button>
            <button class="nh-action-btn secondary" type="button" data-nh-reset>Сброс позиции</button>
            <button class="nh-action-btn secondary" type="button" data-nh-collapse>Свернуть</button>
          </div>
        </div>
        <div class="nh-body">
          <div class="nh-tabs">
    ...
    function updateCollapseLabel() {
      if (!root) return;
      const btn = root.querySelector("[data-nh-collapse]");
      if (!btn) return;
      const collapsed = root.classList.contains("nh-collapsed");
    ...
    function init() {
      ...
      setTimeout(() => {
        Core.Updater && Core.Updater.check && Core.Updater.check();
      }, 500);
    }

    return { init, renderLog, showGuide, hideGuide };
  })();
  ...
  SH.Modules.Blog = (function () {
    ...
    const BLOG_TYPE_DEFAULTS = ["Статья", "Новость", "Видео с экспертами"];
    ...
    function detectType(text) {
      ...
      if (n.includes("новост")) return "Новость";
      return "Статья";
    }
  ...
  SH.Modules.Banners = (function () {
    ...
    function applyToForm(meta) {
      ...
      const setDisclaimerColor = (val) => {
        if (!val) return false;
        const selectors = [
          'input[name="entity_data[disclaimer_color]"]',
          'input[name="entity_data[label_color]"]',
          'input[name="entity_data[disclaimer]"]',
        ];
        for (const sel of selectors) {
          ...
          if (el) {
            el.value = val;
            Dom.dispatchChange(el);
            Log.info("DOM", `Цвет дисклеймера заполнен через ${sel} => ${val}`);
            return true;
          }
        }
        if (Dom.fillInputByLabel("Цвет дисклеймера", val)) return true;
        return Dom.fillInputByLabel("Дисклеймер", val);
      };
